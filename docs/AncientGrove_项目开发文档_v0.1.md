...
| 开发人员 | 个人开发 |
| 当前阶段 | v0.6 打斗精修推进中，已完成 3 段连击、命中检测、Boss 受击硬直、击退、分段命中音效、分段挥击音效、命中特效初版、命中镜头轻微抖动和攻击期间角色转向 |
...
44. 已新增 v0.6 打斗精修版本规划，用于后续完善 3 段轻攻击连击、命中窗口、敌人受击硬直、击退、死亡和基础打击反馈。
45. 已新增 `CS_HitImpact_Small` 命中镜头轻微抖动资源，并接入 `BP_PlayerCharacter.PlayHitFeedback`。
46. 已在 `AM_Player_Attack_Combo` 中接入 3 段挥击音效 `SFX_Swing_LightPunch_01`、`SFX_Swing_HeavyPunch_01` 和 `SFX_Swing_Kick_01`，空挥与命中测试通过。
47. 已接入攻击起手、命中、收招三阶段转向控制，并通过 `Allow Physics Rotation During Anim Root Motion` 修复 Root Motion Montage 下攻击转向不生效的问题。

当前状态：

```text
v0.5 宝箱交互基础目标已完成。
v0.6 打斗精修已进入战斗手感补强阶段。BP_PlayerCharacter 中 IA_Attack 与 IA_Interact 的职责已拆分，HUD 血量刷新、Boss 战 HUD 血条、Boss 追踪和攻击分支均已完成基础校对。当前 v0.6 已完成 3 段连击、Montage Section、Anim Notify 连击窗口、Sphere Trace 命中检测、每段只命中一次、分段伤害、Boss 受击硬直、停止移动、击退、分段命中音效、分段挥击音效、命中特效初版、命中镜头轻微抖动和攻击期间角色转向。下一步以 v0.6 独立文档为准，优先处理攻击滑步和攻击期间移动手感。
```
...
23. 通关流程确认稳定后，进入 v0.6 打斗精修；
24. v0.6 第一阶段先创建或整理 `AM_Player_Attack_Combo`，加入 3 段攻击 Section；
25. v0.6 第二阶段再接入 Anim Notify 命中窗口和 Sphere Trace 命中检测；
26. v0.6 第三阶段再补充敌人受击硬直、击退、死亡和打击反馈。
...
| 2026-06-25 | v0.6 Boss 受击硬直、停止移动和击退 | [2026-06-25.md](devlog/2026-06-25.md) |
| 2026-06-26 | v0.6 命中镜头轻微抖动接入 | [2026-06-26.md](devlog/2026-06-26.md) |
| 2026-06-29 | v0.6 临时挥击音效、攻击期间角色转向接入和测试通过 | [2026-06-29.md](devlog/2026-06-29.md) |
...
```text
v0.6 不优先增加大量新系统，而是精修玩家攻击、敌人受击和打击反馈。
当前实现方式优先使用蓝图、Animation Montage、Anim Notify 和 Sphere Trace。
```

功能：

1. 玩家攻击支持 3 段轻攻击连击；
2. 使用 Animation Montage 管理攻击段落；
3. 使用 Anim Notify 控制真正造成伤害的命中窗口；
4. 使用 Sphere Trace By Channel 检测攻击是否命中敌人；
5. 每段攻击对同一个敌人只造成一次伤害；
6. 每段攻击可设置不同伤害，第三段攻击击退更明显；
7. 敌人被击中后会掉血、受击硬直、短暂击退；
8. 敌人血量归零后触发死亡流程；
9. 攻击时降低移动速度，减少攻击动画滑步；
10. 增加基础打击反馈：挥击音效、命中特效、镜头轻微抖动和短暂停顿；
11. 攻击期间按起手、命中、收招阶段控制角色转向。
...
|---|---|---:|---|
| `IsAttacking` | Boolean | false | 是否正在攻击 |
| `CanCombo` | Boolean | false | 是否允许接下一段攻击 |
| `HasQueuedCombo` | Boolean | false | 是否已经缓存下一次攻击输入 |
| `AttackIndex` | Integer | 0 | 当前攻击段数 |
| `MaxCombo` | Integer | 3 | 最大连击数 |
| `AttackDamage` | Float | 20 | 基础伤害 |
| `AttackTraceRadius` | Float | 35 | 攻击命中检测半径 |
| `HitActors` | Actor Array | 空 | 本段攻击已经命中过的对象，避免重复扣血 |
...
| `HitStunTime` | Float | 0.25 | 受击硬直持续时间 |
| `HitKnockbackStrength` | Float | 450 | 水平击退强度 |
| `HitKnockbackUpStrength` | Float | 80 | 向上击退强度 |

推荐实现顺序：

1. 创建或整理 `AM_Player_Attack_Combo`；
2. 在 `AM_Player_Attack_Combo` 中创建 3 个 Section：`Attack_1`、`Attack_2`、`Attack_3`；
3. 在玩家动画蓝图中确认 `Slot 'DefaultSlot'` 已接在移动状态机和 `Output Pose` 之间；
4. 调整 `IA_Attack` 输入逻辑，支持开始攻击和缓存连击输入；
5. 在 Montage 中加入 `AttackHitStart`、`AttackHitCheck`、`ComboWindowOpen`、`ComboWindowClose`、`AttackEnd`；
6. 在 `BP_PlayerCharacter` 中实现 `AttackHitCheck`，使用 `Sphere Trace By Channel` 检测前方敌人；
7. 在 `BP_AncientGuardian` 中整理受击入口，后续可命名为 `ReceiveHit`；
8. 实现敌人掉血、硬直、击退和死亡流程；
9. 补充挥击音效、命中特效、镜头轻微抖动；
10. 接入攻击期间角色转向，先处理起手转向、命中锁定和收招修正；
11. 测试攻击时移动速度降低，确认攻击动画不再明显滑步。

蓝图操作注意：

```text
如果蓝图中出现多个同名节点，应按上下文区分。
玩家攻击命中检测使用 BP_PlayerCharacter 中的 AttackHitCheck。
敌人受击处理使用 BP_AncientGuardian 中的受击逻辑，不要误接到 WBP_PlayerHUD 的血条刷新函数。
Boss 血条刷新仍由 BP_AncientGuardian 通过 BP_PlayerCharacter 转发到 WBP_PlayerHUD。
```
...
```text
创建动画蒙太奇：
内容浏览器（Content Browser）-> 右键（Right Click）-> 动画（Animation）-> 动画蒙太奇（Animation Montage）

打开动画蓝图：
内容浏览器（Content Browser）-> 双击玩家动画蓝图（Double Click Player Animation Blueprint）
...
```text
1. 鼠标左键可以打出 3 段连击；
2. 不在连击窗口按攻击，不会乱触发下一段；
3. 在连击窗口按攻击，可以自然接下一段；
4. 每段攻击只会对同一个敌人造成一次伤害；
5. 敌人被打中会掉血、受击、短暂停顿；
6. 第三段攻击击退更明显；
7. 敌人血量归零后死亡；
8. 攻击动画不再明显滑步；
9. 命中时有音效、特效、镜头轻微抖动；
10. Boss 战 HUD 血条仍能随 Boss 血量变化正常刷新。
```
...
截至 2026-06-26，分段命中音效、命中特效初版和命中镜头轻微抖动也已测试通过。
截至 2026-06-29，3 段挥击音效、攻击期间角色转向接入和 Root Motion 转向修复已测试通过。
